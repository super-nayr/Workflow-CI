...
def load_data(data_path: str) -> tuple:
    ...
    # Menggunakan nama kolom 'lung_cancer' (huruf kecil) sesuai screenshot CSV Anda
    target_column = "lung_cancer" 
    
    if target_column not in df.columns:
        print(f"::error::Kolom target '{target_column}' tidak ditemukan di dataset Anda.")
        exit(1)
...
def train_model(X_train, X_test, y_train, y_test, max_iter: int, C: float):
    ...
    with mlflow.start_run():
        ...
        # Mencetak Run ID agar bisa ditangkap oleh GitHub Actions
        ...
        # Log model ke path 'model_files' untuk menghindari masalah API DagsHub
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train a Lung Cancer Prediction Model.")
